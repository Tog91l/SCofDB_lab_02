...
Race condition (состояние гонки) — это ситуация, когда корректность работы программы зависит от порядка выполнения операций в параллельных процесах.

**Пример из жизни:**
Заказ оплачивается дважны
...
1. Каждая транзакция видит закомиченные данные на каждый запрос 
2. Когда две транзакции одновременно то обе меняют статус на другой(например как в лабе, заказ оплачен дважды) 
3. В результате заказ оплачен дважды
...
2. **Микросервисная архитектура**
   - одновременное получение события
   - начинается обработа платежа
   - конфликт и соотвественно две оплаты на один заказ

3. **Сбой сети**
   - запрос об оплате
   - сбой сети
   - повторный запрос
   - обработка того же запроса второй раз
