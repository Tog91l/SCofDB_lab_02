...
class PaymentService:
    ...
    async def pay_order_unsafe(self, order_id: uuid.UUID) -> dict:
        ...
        async with self.session.begin():
            stat_now = await self.session.execute(text("SELECT status FROM orders WHERE id = :order_id"),{"order_id": order_id})
            ...
            await self.session.execute(
                text("""INSERT INTO order_status_history (id, order_id, status, changed_at)
